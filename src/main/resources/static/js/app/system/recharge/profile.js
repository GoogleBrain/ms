...
$(function () {
    ...
    $(".default_avatars_list").find("img").each(function () {
        ...
        $this.on("click", function () {
            ...
            $.post(ctx + "recharge/changeAvatar", {"imgName": target_src}, function (r) {
                if (r.code === 0) {
    ...
    $("#update-profile .btn-save").click(function () {
        ...
        if (flag) {
            $.post(ctx + "recharge/updateUserProfile", $userProfileForm.serialize(), function (r) {
                if (r.code === 0) {
...
function refreshUserProfile() {
    $.post(ctx + "recharge/profile", function (r) {
        $main_content.html("").append(r);
    });
}

function editUserProfile() {
    $.post(ctx + "recharge/getUserProfile", {"userId": userId}, function (r) {
        if (r.code === 0) {
