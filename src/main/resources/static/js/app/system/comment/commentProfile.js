...
$(function () {
    ...
    $(".default_avatars_list").find("img").each(function () {
        ...
        $this.on("click", function () {
            ...
            $.post(ctx + "comment/changeAvatar", {"imgName": target_src}, function (r) {
                if (r.code === 0) {
    ...
    $("#update-profile .btn-save").click(function () {
        ...
        if (flag) {
            $.post(ctx + "comment/updateUserProfile", $userProfileForm.serialize(), function (r) {
                if (r.code === 0) {
...
function refreshUserProfile() {
    $.post(ctx + "comment/profile", function (r) {
        $main_content.html("").append(r);
    });
}

function editUserProfile() {
    $.post(ctx + "comment/getUserProfile", {"userId": userId}, function (r) {
        if (r.code === 0) {
