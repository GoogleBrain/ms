...
function deleteUsers() {
    ...
    $MB.confirm({
        ...
        $.post(ctx + 'rebate/delete', {"ids": ids}, function (r) {
            if (r.code === 0) {
                $MB.n_success(r.msg);
                refresh();
            } else {
...
function exportUserExcel() {
    $.post(ctx + "rebate/excel", $(".user-table-form").serialize(), function (r) {
        if (r.code === 0) {
...
function exportUserCsv() {
    $.post(ctx + "rebate/csv", $(".user-table-form").serialize(), function (r) {
        if (r.code === 0) {
            window.location.href = "common/download?fileName=" + r.msg + "&delete=" + true;
        } else {
            $MB.n_warning(r.msg);
